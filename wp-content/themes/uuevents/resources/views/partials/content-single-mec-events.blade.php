@php
    ...
    $more_info_button_text = $event->meta['mec_more_info_title']
        ? $event->meta['mec_more_info_title']
        : 'More Information';
    ...
    $event_button =
        '<a class="no-underline mt-6 button button-solid bg-action hover:bg-action-dark items-center gap-4" href="' .
        $more_info .
        '"' .
        $target .
                        ...
                        @foreach ($event->locations as $location)
                            <span class="flex gap-4 items-center">
                                <span class="w-7 h-7 flex items-center">
                                    <svg class="w-full h-full" viewBox="0 0 31 31" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path fill-rule="evenodd" clip-rule="evenodd"
                                            d="M22.9861 6.57538C27.0866 10.6759 27.0866 17.3241 22.9861 21.4246L15.5615 28.8493L8.1369 21.4246C4.0364 17.3241 4.0364 10.6759 8.1369 6.57538C12.2374 2.47487 18.8856 2.47487 22.9861 6.57538ZM15.5615 17C17.2184 17 18.5615 15.6569 18.5615 14C18.5615 12.3432 17.2184 11 15.5615 11C13.9047 11 12.5615 12.3432 12.5615 14C12.5615 15.6569 13.9047 17 15.5615 17Z"
                                            fill="#216CFF" />
                                    </svg>
                                </span>
                                {{ $location['name'] }}</span>
...
@if ($event->post->post_content)
    <div
        class="section-divider relative h-5 sm:h-10 md:h-14 xl:h-20 -mb-2 -sm:mb-3 md:-mb-7 xl:-mb-10 text-light-gradient">
            ...
            <div class="flex flex-wrap justify-between items-center gap-2 mb-6 md:mb-10">
                ...
                <a href="/" class="text-action no-underline font-semibold flex gap-2 items-center justify-evenly">
                    ...
                    <svg width="14" height="12" viewBox="0 0 14 12" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path d="M8.43652 1.875L12.5615 6M12.5615 6L8.43652 10.125M12.5615 6L1.56152 6" stroke="#216CFF"
                    ...
                    <div class="relative group bg-white border border-light shadoew-lg rounded-lg overflow-hidden">
                        ...
                        <article class=" mec-event-article mec-clear  " itemscope="">
                            ...
                            <div
                                ...
                                <span class="text-brand uppercase">{{ $start_month }}</span>
                            </div>
                            <div class="px-7 pb-10">
                                <h3 class="mb-4 text-[28px] flex flex-wrap gap-2">
                                    <a class="no-underline text-brand group-hover:text-action"
                                        data-event-id="{{ $event->ID }}" href="{{ $data->permalink }}"
                                ...
                                <div class="flex flex-wrap gap-2 relative z-20">
                                    <div
                                        class="inline-flex mt-4 gap-2 px-2 py-1 justify-center items-start no-underline text-brand hover:shadow-inner border-2 border-pale-blue-dark rounded-[16px]">
                                        <span>
                                        ...
                                        <span class="text-brand text-sm">
                                            {!! str_replace(
                                                'mec-time-details',
                                                '',
                                                MEC_kses::element($main->display_time($data->time['start'], $data->time['end'])),
                                            ) !!}
                                    ...
                                    @isset($data->labels)
                                        @foreach ($data->labels as $label)
                                            <span class="mec-labels-normal relative z-20">
                                                <span data-style="Normal" class="tw-mec-label-normal"
                        ...
                        <a class="no-underline mec-color-hover absolute inset-0 z-40" data-event-id="{{ $event->ID }}"
                            href="{{ $data->permalink }}" target="_self" rel="noopener">
